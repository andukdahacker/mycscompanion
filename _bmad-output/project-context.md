...
_Critical rules and patterns for AI agents implementing code in mycscompanion. One page. Read before every story._
...
- `apps/website` — Astro (static), React islands from `@mycscompanion/ui`
- `apps/backend` — Fastify + pino (structured JSON logging), Kysely, BullMQ

**Shared Packages (internal — TypeScript source, NO build step):**
- `@mycscompanion/ui` — shadcn/ui + Tailwind CSS v4 preset
- `@mycscompanion/shared` — types, constants, `toCamelCase()`. NO runtime deps. `src/types/db.ts` is generated by `kysely-codegen` (gitignored — run `pnpm --filter shared db:types` after migrations)
- `@mycscompanion/execution` — Execution service client, typed SSE events (discriminated union), benchmark runner
- `@mycscompanion/config` — ESLint, TS base, Vitest base, canonical test utilities + mock factories

**Infrastructure:** PostgreSQL 16, Redis 7 (via ioredis — NOT the `redis` npm package), Go 1.23 (execution server)

**External Services:** Firebase Auth, Anthropic API (Haiku 4.5 default / Sonnet 4.6 for code analysis), Fly.io (execution service hosting), Sentry
...
**Import Paths:**
- Internal packages: `import { ... } from '@mycscompanion/shared'` — configured in `tsconfig.json` `paths`
...
- Exception: `@mycscompanion/ui` has NO barrel file — import components individually for tree-shaking
...
**`apiFetch` location:** `apps/webapp/src/lib/api-fetch.ts` — NOT in `@mycscompanion/shared` (depends on Firebase client SDK).
...
Pure static. No auth, no runtime API calls. Reads `content/milestones/01-kv-store/` at build time for Milestone 1 preview. React islands from `@mycscompanion/ui` for interactive components. `font-display: swap` (prioritize LCP).
...
|---|---|
| Execution service | `msw` v2 HTTP handlers |
...
**Import from `@mycscompanion/config/test-utils/`** — never create ad-hoc mocks. New patterns → add to canonical set.
...
| Env vars (custom) | `MCC_` prefix | `MCC_EXECUTION_URL` (third-party keep standard: `ANTHROPIC_API_KEY`) |
...
- Barrel import from `@mycscompanion/ui` — import components individually
